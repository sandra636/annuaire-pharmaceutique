        ...
        jQuery.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'online_pharmacy_dismissed_notice_handler',
              type: type,
              wpnonce: online_pharmacy.wpnonce
            }
          } );
      } );
}( jQuery ) )
    ...
    if (closeBtn && detailBox) {
        closeBtn.addEventListener("click", function(e) {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=online_pharmacy_dismissed_get_started_detail_notice'
            })
...
function online_pharmacy_open_tab(evt, tabName) {
    let i, tabcontent, tablinks;

    // Hide all tabs
    tabcontent = document.getElementsByClassName("online-pharmacy-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Remove active from all buttons
    tablinks = document.getElementsByClassName("online-pharmacy-tablinks");
...
jQuery(document).ready(function ($) {
    // Hide all tabs first
    $(".online-pharmacy-tabcontent").hide();
    
    // **DIRECTLY** activate first tab (no trigger)
    let firstTab = $(".online-pharmacy-tablinks").first();
